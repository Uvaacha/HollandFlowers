...
import './TulipsArrangement.css';

const TulipsArrangement = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const tulipsCategory = categories.find(cat => {
          const name = (cat.categoryName || cat.nameEn || cat.name || '').toLowerCase();
          return name === 'tulips' || name === 'tulips arrangement' || 
                 name === 'tulip' || name.includes('tulip');
        });

        if (!tulipsCategory) {
          console.warn('Category not found. Available:', categories.map(c => c.categoryName));
          setError('Category not found. Please create "Tulips" category in admin.');
          setProducts([]);
          setLoading(false);
          return;
        }

        const productsResponse = await productService.getProductsByCategory(tulipsCategory.categoryId, {
          page: 0,
          size: 100,
          sort: 'createdAt,desc'
        });
  ...
  const translations = {
    en: {
      pageTitle: "Tulips Collection",
      pageSubtitle: "Elegant tulips in stunning colors and arrangements",
      filters: "Filters",
      ...
      loading: "Loading tulips collection...",
      noProducts: "No tulips products available yet.",
      error: "Something went wrong",
      freshBanner: "Fresh Tulips from Holland",
      freshBannerDesc: "Premium imported tulips in vibrant colors. Perfect for any occasion with same-day delivery available."
    },
    ar: {
  ...
  return (
    <div className={`tulips-page ${currentLang === 'ar' ? 'rtl' : ''}`}>
      {/* Decorative Background */}
      <div className="tulips-bg">
        <div className="tulip-glow glow-1"></div>
        <div className="tulip-glow glow-2"></div>
        <div className="floating-tulip ft-1">🌷</div>
        <div className="floating-tulip ft-2">💐</div>
        <div className="floating-tulip ft-3">🌷</div>
        <div className="floating-tulip ft-4">💮</div>
      </div>

      {/* Hero Section */}
      <section className="tulips-hero">
      ...
      <section className="tulips-content">
        <div className="container">
...
export default TulipsArrangement;