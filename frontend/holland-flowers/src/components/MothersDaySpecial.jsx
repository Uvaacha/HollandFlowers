...
import './MothersDaySpecial.css';
import AddToCartModal from './AddToCartModal';

const MothersDaySpecial = () => {
  ...
  const translations = {
    en: {
      title: "Mother's Day Specials", subtitle: "Celebrate the most special woman in your life", badge: "For Dear Mom",
      ...
      loading: "Loading Mother's Day collection...", error: "Failed to load products", noProducts: "No products found", highestPrice: "Highest price",
      color: 'COLOR'
    },
    ar: {
      title: "عروض عيد الأم", subtitle: "احتفلي بأغلى امرأة في حياتك", badge: "للأم العزيزة",
      ...
      loading: "جاري تحميل مجموعة عيد الأم...", error: "فشل في تحميل المنتجات", noProducts: "لا توجد منتجات", highestPrice: "أعلى سعر",
      color: 'اللون'
    }
  };
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const motherCategory = categories.find(cat => {
          const name = (cat.categoryName || cat.nameEn || cat.name || '').toLowerCase();
          return name === "mother's day" || name === 'mothers day' || name.includes('mother');
        });
        if (!motherCategory) { setError('Category not found.'); setProducts([]); setLoading(false); return; }
        const productsResponse = await productService.getProductsByCategory(motherCategory.categoryId, { page: 0, size: 100, sort: 'createdAt,desc' });
        let productsList = [];
  ...
  return (
    <div className={`mothers-page ${currentLang === 'ar' ? 'rtl' : ''}`}>
...
export default MothersDaySpecial;