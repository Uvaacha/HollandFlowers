...
import './FlowersWithMabkhar.css';

const FlowersWithMabkhar = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Find Flowers With Mabkhar category - flexible search
        const mabkharCategory = categories.find(cat => {
          const name = (cat.categoryName || cat.nameEn || cat.name || '').toLowerCase();
          return name === 'flowers with mabkhar' || name === 'flowers mabkhar' || 
                 name === 'mabkhar' || name.includes('mabkhar');
        });

        if (!mabkharCategory) {
          console.warn('Category not found. Available:', categories.map(c => c.categoryName));
          setError('Category not found. Please create "Flowers With Mabkhar" category in admin.');
          setProducts([]);
          setLoading(false);
          return;
        }

        console.log('Found category:', mabkharCategory);

        const productsResponse = await productService.getProductsByCategory(mabkharCategory.categoryId, {
          page: 0,
          size: 100,
          sort: 'createdAt,desc'
        });
  ...
  const translations = {
    en: {
      pageTitle: "Flowers With Mabkhar",
      pageSubtitle: "Elegant flower arrangements with traditional incense burners",
      breadcrumbHome: "Home",
      breadcrumbFlowers: "Flowers",
      breadcrumbCurrent: "Flowers With Mabkhar",
      addToCart: "Add to Cart",
      currency: "KD",
      trayBadge: "Tray Set",
      boxBadge: "Box Set",
      premiumBadge: "Premium",
      loading: "Loading flowers with mabkhar...",
      noProducts: "No flowers with mabkhar available yet.",
      error: "Something went wrong",
    },
    ar: {
      pageTitle: "زهور مع مبخر",
      pageSubtitle: "ترتيبات زهور أنيقة مع مباخر تقليدية",
      breadcrumbHome: "الرئيسية",
      breadcrumbFlowers: "الزهور",
      breadcrumbCurrent: "زهور مع مبخر",
      addToCart: "أضف للسلة",
      currency: "د.ك",
      trayBadge: "طقم صينية",
      boxBadge: "طقم صندوق",
      premiumBadge: "مميز",
      loading: "جاري تحميل الزهور مع المبخر...",
      noProducts: "لا توجد زهور مع مبخر متاحة حالياً.",
      error: "حدث خطأ ما",
    }
  };
  ...
  const getProductBadge = (product) => {
    ...
    if (tags.includes('premium') || name.includes('stair') || name.includes('queen') || name.includes('stand') || product.isPremium) return 'premium';
  ...
  return (
    <div className={`flowers-mabkhar-page ${currentLang === 'ar' ? 'rtl' : ''}`}>
      <div className="flowers-mabkhar-bg">
        <div className="fm-glow glow-1"></div>
        <div className="fm-glow glow-2"></div>
        <div className="floating-fm ffm-1">🕯️</div>
        <div className="floating-fm ffm-2">🌹</div>
        <div className="floating-fm ffm-3">✨</div>
        <div className="floating-fm ffm-4">🎁</div>
      </div>
        ...
        <div className="container">
          <div className="hero-content">
            <div className="hero-badge">
              <span>🕯️</span>
              <span>{currentLang === 'ar' ? 'عطور تقليدية' : 'Traditional Fragrance'}</span>
              <span>🕯️</span>
            </div>
            ...
            <div className="hero-stats">
              ...
              <div className="stat">
                ...
                <span className="stat-label">{currentLang === 'ar' ? 'مبخر' : 'Mabkhar'}</span>
              </div>
              <div className="stat-divider"></div>
              <div className="stat">
                <span className="stat-number">🌹</span>
                <span className="stat-label">{currentLang === 'ar' ? 'زهور' : 'Flowers'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="container">
          ...
          {loading && (
            <div className="loading-state" style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <p style={{ fontFamily: 'Poppins, sans-serif', fontSize: '14px', color: '#b8956c' }}>{t.loading}</p>
          ...
          {error && !loading && (
            <div className="error-state" style={{ 
              ...
              <p style={{ fontFamily: 'Poppins, sans-serif', fontSize: '13px', color: '#b8956c' }}>{error}</p>
            </div>
          )}
          ...
          {!loading && !error && products.length === 0 && (
            <div className="empty-state" style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <p style={{ fontFamily: 'Poppins, sans-serif', fontSize: '16px', color: '#b8956c' }}>{t.noProducts}</p>
            </div>
          )}
                  ...
                  >
                    ...
                    <div className="product-details">
                      ...
                      <p className="product-desc">
                        {productDesc || (currentLang === 'ar' ? 'زهور مع مبخر تقليدي' : 'Flowers with traditional mabkhar')}
                      </p>
        ...
        <div className="container">
          <div className="banner-content">
            ...
            <div className="banner-text">
              <h3>{currentLang === 'ar' ? 'أجواء عربية أصيلة' : 'Authentic Arabian Ambiance'}</h3>
              <p>{currentLang === 'ar' ? 'زهور فاخرة مع مباخر تقليدية. مثالية للمناسبات العربية والضيافة الراقية' : 'Premium flowers with traditional incense burners. Perfect for Arabian occasions and elegant hospitality'}</p>
            </div>
          </div>
        </div>
      </section>
...
export default FlowersWithMabkhar;