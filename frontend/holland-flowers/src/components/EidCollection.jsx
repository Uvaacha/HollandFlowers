...
const EidCollection = () => {
  ...
  const translations = {
    en: {
      title: "Eid Collection", subtitle: "Celebrate Eid with our exclusive flower arrangements", badge: "Eid Mubarak",
      ...
      loading: "Loading Eid collection...", error: "Failed to load products", noProducts: "No products found", highestPrice: "Highest price",
      color: 'COLOR'
    },
    ar: {
      title: "مجموعة العيد", subtitle: "احتفل بالعيد مع تنسيقات الزهور الحصرية", badge: "عيد مبارك",
      ...
      loading: "جاري تحميل مجموعة العيد...", error: "فشل في تحميل المنتجات", noProducts: "لا توجد منتجات", highestPrice: "أعلى سعر",
      color: 'اللون'
    }
  };
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const eidCategory = categories.find(cat => {
          const name = (cat.categoryName || cat.nameEn || cat.name || '').toLowerCase();
          return name === 'eid' || name === 'eid collection' || name.includes('eid');
        });
        if (!eidCategory) { setError('Category not found.'); setProducts([]); setLoading(false); return; }
        const productsResponse = await productService.getProductsByCategory(eidCategory.categoryId, { page: 0, size: 100, sort: 'createdAt,desc' });
        let productsList = [];
  ...
  return (
    <div className={`eid-page ${currentLang === 'ar' ? 'rtl' : ''}`}>
