...
const ValentineSpecial = () => {
  ...
  const translations = {
    en: {
      title: "Valentine's Day Collection", subtitle: "Express your love with our romantic flower arrangements", badge: "Love Collection",
      ...
      loading: "Loading romantic collection...", error: "Failed to load products", noProducts: "No products found", highestPrice: "Highest price",
      color: 'COLOR'
    },
    ar: {
      title: "مجموعة عيد الحب", subtitle: "عبّر عن حبك مع تنسيقات الزهور الرومانسية", badge: "مجموعة الحب",
      ...
      loading: "جاري تحميل المجموعة الرومانسية...", error: "فشل في تحميل المنتجات", noProducts: "لا توجد منتجات", highestPrice: "أعلى سعر",
      color: 'اللون'
    }
  };
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const valentineCategory = categories.find(cat => {
          const name = (cat.categoryName || cat.nameEn || cat.name || '').toLowerCase();
          return name === 'valentine' || name === "valentine's day" || name.includes('valentine');
        });
        if (!valentineCategory) { setError('Category not found.'); setProducts([]); setLoading(false); return; }
        const productsResponse = await productService.getProductsByCategory(valentineCategory.categoryId, { page: 0, size: 100, sort: 'createdAt,desc' });
        let productsList = [];
  ...
  return (
    <div className={`valentine-page ${currentLang === 'ar' ? 'rtl' : ''}`}>
      <section className="hero-banner">
        <div className="hero-bg-decoration"><span className="floating-heart float-1">❤️</span><span className="floating-heart float-2">💕</span><span className="floating-heart float-3">💖</span></div>
                    ...
                    return (
                      <Link to={`/product/${productSlug}`} className="product-card" key={productSlug} style={{ animationDelay: `${index * 0.05}s` }}>
                        ...
                        <div className="product-info">
                          ...
                          <p className="product-desc">{currentLang === 'ar' ? 'باقة رومانسية جميلة' : 'Beautiful romantic arrangement'}</p>
